...
def load_conv(buf, start, conv_model):
    ...
    if '0.3' == torch.__version__[:3] or '0.2' == torch.__version__[:3]:
        ...
        conv_model.weight.data.copy_(torch.reshape(torch.from_numpy(buf[start:start+num_w]),(conv_model.weight.shape[0],conv_model.weight.shape[1], conv_model.weight.shape[2],conv_model.weight.shape[3]))); start=start + num_w #20180615 modified for >= torch v0.4.0
    return start
